...
public class CommandSubsystem {
    ...
    public boolean interpretCommand(CommandSender sender, String label, String[] args) {
    	...
    	if (label.equalsIgnoreCase("prof")) {
    		...
    		pc.interpret(sender, label, args);
    		return true;
    	}
        // mf commands
        if (label.equalsIgnoreCase("mf") || label.equalsIgnoreCase("f") ||
            ...
            if (args.length > 0) {
                        ...
                        if (sender instanceof Player) {
                            ...
                            if (isInFaction(player.getUniqueId(), main.factions)) {
                                ...
                                if(main.allTradePosts.containsKey(c)) {
                                	TradePoint p = main.allTradePosts.get(c);
                                	Faction f = main.utilities.getFaction(p.factionName, main.factions);
                                	f.removeAfterUnclaim(p);
                                }
                                return true;
                            }
                            else {
                ...
                if (args[0].equalsIgnoreCase("tradepoint") || args[0].equalsIgnoreCase("trp")) {               	
                	TradePointCommand command = new TradePointCommand(main);
