...
public class UtilitySubsystem {
    ...
    public boolean isFurnace(Block block)
    {
    	if (block.getType() == Material.FURNACE ||
    			block.getType() == Material.BURNING_FURNACE) // BLAST_FURNACE
    	{
    		return true;
    	}
    	return false;
    ...
    public static void sendFactionInfo(Player player, Faction faction, int power) {
    	...
    	long inverse = 86400000-elapsedTime;
    	if(elapsedTime >= 86400000) cooldown = "Доступен сейчас!";
    	else cooldown = "Доступен через " + String.format("%d минут, %d секунд", 
    		    TimeUnit.MILLISECONDS.toMinutes(inverse),
    		    TimeUnit.MILLISECONDS.toSeconds(inverse) - 
    		    TimeUnit.MINUTES.toSeconds(TimeUnit.MILLISECONDS.toMinutes(inverse))
    		);
        ...
        player.sendMessage(ChatColor.AQUA + "Военная репутация: " + faction.getWarReputationState().toString() + "(" + 
        faction.getWarReputation()+ ")" + "\n");
        player.sendMessage(ChatColor.AQUA + "Торговая репутация: " + faction.getTradeReputation() + "\n");
        
        player.sendMessage(ChatColor.AQUA + "Дипломатический посол фракции: " + cooldown + "\n");
