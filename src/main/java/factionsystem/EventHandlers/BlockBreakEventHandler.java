    ...
    public void handle(BlockBreakEvent event) {
        ...
        if (chunk != null) {
        	...
        	for (Faction f : main.factions) {
        		if(f.getName().equalsIgnoreCase(chunk.getHolder())) {
        			myFaction = f;
        			break;
        		}
        	}
        	
        	// If warlord/attacked friendly, nothing will help
        	if(myFaction.getWarReputation() < -40) {
        		player.sendMessage(ChatColor.GREEN + "За разграбление завоевателя вам ничего не будет!");
    			return;
        	}
        	if(myFaction.hasFriendlyEnemy()) {
        		player.sendMessage(ChatColor.GREEN + "За разграбление тех, кто посягает на дружелюбных, ничего не будет!");
        		return;
        	}        	
